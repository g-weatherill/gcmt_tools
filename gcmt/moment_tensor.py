...
"""
SEt of utility functions for moment tensor calculations
"""
...
def unique_euler( alpha, beta, gamma ):
    ...
    Put euler angles into ranges compatible with (dip,strike,-rake) in
    seismology:
    alpha (dip) : [0, pi/2]
    beta (strike) : [0, 2*pi)
    gamma (-rake) : [-pi, pi)
...
class MomentTensor:
    ...
    _flip_dc = num.matrix([[0.,0.,-1.],[0.,-1.,0.],[-1.,0.,0.]], 
                           dtype=num.float )
    _to_up_south_east = num.matrix([[0.,0.,-1.],[-1.,0.,0.],[0.,1.,0.]], 
                                   dtype=num.float ).T
    _m_unrot = num.matrix([[0.,0.,-1.],[0.,0.,0.],[-1.,0.,0.]], 
                          dtype=num.float )
    _u_evals, _u_evecs = eigh_check(_m_unrot)

    def __init__(self, m=None, m_up_south_east=None, strike=0., dip=0., 
                 rake=0., scalar_moment=1. ):
        '''Create moment tensor object based on 3x3 moment tensor matrix or 
        orientation of fault plane and scalar moment.
        In:
        ...
        self._rotmats = sorted([rotmat1, MomentTensor._flip_dc * rotmat1], 
                               cmp=cmp_mat)
    ...
    def __str__(self):
        ...
        s = s % (self.scalar_moment(), self.moment_magnitude(),
                 m[0,0],m[1,1],m[2,2],m[0,1],m[0,2],m[1,2], mexp)
    ...
    def str_fault_planes(self):
        ...
        for i,sdr in enumerate(self.both_strike_dip_rake()):
            s += 'Fault plane %i: strike = %3.0f, dip = %3.0f, '\
...
class MomentTensorTestCase( unittest.TestCase ):
    ...
    def testProblematicAngles(self):
        ...
        for i in range(100):
            ...
            (s1,d1,r1) = [r2d*random.randint(-16,16)*
                math.pi/8.+1e-8*random.random()-0.5e-8 for j in range(3)]
            m0 = 1.+random.random()*1.0e20
            self.forwardBackward( s1, d1, r1, m0 )
    ...
    def testNonPlainDoubleCouples(self):
        ...
        for i in range(100):
            ...
            m = num.matrix([[ ms[0],ms[3],ms[4]],[ms[3],
                ms[1],ms[5]],[ms[4],ms[5],ms[2]]], dtype=num.float )
    ...
    def forwardBackward(self, strike, dip, rake, scalar_moment ):
        m1 = MomentTensor( strike=strike, dip=dip, rake=rake, 
    ...
    def testChile(self):
        m_use = symmat6(1.040, -0.030, -1.010, 0.227, -1.510, -0.120)\
            *1e29*dynecm
